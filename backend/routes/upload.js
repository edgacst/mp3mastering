const express = require('express');
const multer = require('multer');
const path = require('path');
const fs = require('fs');
const { fixEncoding, pickBetterFilename } = require('../lib/fixFilenameEncoding');
const { createRateLimiter } = require('../lib/rateLimit');

const router = express.Router();

const uploadDir = path.join(__dirname, '..', 'uploads');
if (!fs.existsSync(uploadDir)) fs.mkdirSync(uploadDir, { recursive: true });

// 브라우저/OS마다 MIME 타입이 다양함 (audio/mp3, audio/x-mpeg 등)
const allowedMime = ['audio/mpeg', 'audio/mp3', 'audio/x-mpeg', 'audio/x-mp3', 'application/octet-stream'];
const uploadRateLimit = createRateLimiter({
  windowMs: 10 * 60 * 1000,
  max: Number(process.env.MASTERING_UPLOAD_LIMIT) || 30,
});

function resolveOriginalName(multerName, clientName) {
  const fixedMulter = fixEncoding(multerName || '');
  const fixedClient = fixEncoding(String(clientName || '').trim());
  return pickBetterFilename(fixedClient, fixedMulter) || fixedMulter || 'track.mp3';
}

const storage = multer.diskStorage({
  destination: uploadDir,
  filename: (req, file, cb) => {
    file.originalname = resolveOriginalName(file.originalname, req.body?.displayName);
    const unique = Date.now() + '-' + Math.round(Math.random() * 1e9);
    cb(null, unique + '.mp3');
  },
});

const upload = multer({
  storage,
  limits: { fileSize: 100 * 1024 * 1024 }, // 100MB (Nginx client_max_body_size와 맞춤)
  fileFilter: (req, file, cb) => {
    file.originalname = resolveOriginalName(file.originalname, req.body?.displayName);
    const ext = path.extname(file.originalname).toLowerCase();
    if (allowedMime.includes(file.mimetype) || ext === '.mp3') {
      cb(null, true);
    } else {
      cb(new Error('MP3 파일만 업로드 가능합니다.'));
    }
  },
});

router.post('/', uploadRateLimit, upload.single('file'), (req, res) => {
  if (!req.file) return res.status(400).json({ error: '파일이 없습니다.' });
  const originalname = resolveOriginalName(req.file.originalname, req.body?.displayName);
  req.file.originalname = originalname;
  res.json({
    filename: req.file.filename,
    originalname,
    size: req.file.size,
  });
});

/** 미리듣기용 원본 스트림 (업로드 파일 유지) */
router.get('/original/:filename', (req, res) => {
  const safeName = path.basename(req.params.filename || '');
  const filePath = path.join(uploadDir, safeName);
  if (!safeName || !fs.existsSync(filePath)) {
    return res.status(404).json({ error: '파일을 찾을 수 없습니다.' });
  }
  res.setHeader('Content-Type', 'audio/mpeg');
  res.setHeader('Content-Disposition', 'inline');
  res.setHeader('Cache-Control', 'no-store');
  res.setHeader('Accept-Ranges', 'bytes');
  const stat = fs.statSync(filePath);
  res.setHeader('Content-Length', String(stat.size));
  fs.createReadStream(filePath).pipe(res);
});

router.use((err, req, res, next) => {
  const msg = String(err.message || err);
  if (err.code === 'ECONNABORTED' || err.type === 'request.aborted' || /request aborted/i.test(msg)) {
    if (!res.headersSent) {
      return res.status(499).json({ error: '업로드가 중단되었습니다. 다시 시도해 주세요.' });
    }
    return;
  }
  res.status(400).json({ error: err.message });
});

module.exports = router;
